...
def load(app):
    # Endpoint: GET /api/groups with pagination
    ...
    def get_groups():
        try:
            ...
            # Query to fetch groups with sorting
            cursor.execute(f'''
                SELECT g.id, g.name
                FROM groups g
    ...
    # Endpoint: GET /api/groups/:id to get a single group with its details
    ...
    def get_group(group_id):
        try:
            ...
            # Query to fetch the group and its details
            cursor.execute('''
                SELECT g.id, g.name
                FROM groups g
                WHERE g.id = ?
